...
\begin{abstract}
We treat the integers as a gas. The single-particle spectrum is the set of
\emph{primons}: one mode per prime $p$, of energy $\log p$. In the usual reading ---
the primon gas of Julia and of Bost--Connes --- an integer is a Fock state (its prime
factorization is an occupation-number configuration) and the Riemann zeta function is
the free bosonic partition function. This paper studies the same gas \emph{before}
symmetrization and \emph{before} the interaction history is forgotten. A microstate is
then a \emph{multiplication tree}: a fireball built by recorded pairwise fusions of
primons, ordered and parenthesized, whose only conserved quantity is the integer
$N(T) = e^{E(T)}$ it evaluates to. Multiplication is not commutative or associative at
this level, because microstates remember their formation; ordinary arithmetic is the
fully coarse-grained description, and the Fundamental Theorem of Arithmetic is
precisely the statement that the occupation-number description is faithful (zero
residual entropy). Forgetting history and symmetrizing are independent
...
distinguishable-particle gas with inverse Hagedorn temperature $1.39943\dots$, a
non-planar fireball gas at $1.98847\dots$, and the full planar fireball gas, whose
partition function obeys the statistical bootstrap equation $Z = P + Z^2$ of
Hagedorn--Frautschi type and reaches its Hagedorn point at
...
axiom. The complex-$\beta$ singularities of the fireball gas (its Fisher zeros) are
computed: they inherit the Riemann zeros through $P$, and their new branch points,
the solutions of $P(\beta) = \tfrac14$, do \emph{not} lie on a vertical line. Finally,
the bridge to addition: fireballs act as \emph{processes} on states of hierarchically
...
object than ``two groups of three'' --- whose measured value happens to be $6$. The
slogan:
...
Physics has a standard name for this situation: $6$ is a \emph{macrostate}, and the
ways of building it are \emph{microstates}. We therefore refuse to let multiplication
collapse into numbers. A product is a \emph{multiplication tree}: the recorded fusion
history of the primes involved, ordered and parenthesized, unevaluated. The familiar
integer is produced only by an evaluator --- the shadow map $N$ --- which plays the
role of the measurement: it reads off the single conserved quantity and discards the
history. The classical laws of multiplication are then not properties of
...
\begin{quote}
\emph{The primes are the particles; an integer is a calorimeter reading; the Riemann
...
multiplication tree $T \in \cT$ & fireball: a recorded history of pairwise fusions \\
...
multisets $\cM$ & occupation numbers: the primon gas of Julia \\
...
critical exponents $\sigma$ & inverse Hagedorn temperatures $\beta_H$ \\
solutions of $P(s) = \tfrac14$ & Fisher singularities: complex-$\beta$ singularities of $Z$ \\
...
\bottomrule
\end{tabular}
\caption{The dictionary. ``Calorimeter'', ``fireball'', ``beam'' and ``cluster'' are
imagery; every other row is an exact identity established in the text.}
...
\begin{enumerate}
\item \textbf{The microstate space} (\S\ref{sec:trees}). The fireball world is the
free magma $\cT$ on the set of primes; the measurement is the unique magma morphism
$N\colon \cT \to (\NN_{\ge2},\cdot)$ with $N(L_p) = p$; energies
$E = \log N$ add under fusion. The interacting world has no vacuum: $1$ is not the
shadow of any fireball, and the empty state exists only in the Fock description.
...
\item \textbf{Four partition functions and a Hagedorn ladder} (\S\ref{sec:zeta}). All
...
fireball gas obeying the bootstrap equation $Z = P + Z^2$. The inverse Hagedorn
temperatures form a strict ladder
\[
1 \;<\; 1.3994333287263303 \;<\; 1.9884768518009081 \;<\; \sigma^{*} = 2.5973851271346716 ,
\]
one rung per layer of remembered structure. At its Hagedorn point the fireball gas
...
have poles: whether the Hagedorn temperature is attainable is decided by
associativity. The classical Hagedorn--Frautschi--Nahm bootstrap, transplanted onto
the primon spectrum, slots into the same family with
$\beta_H = 2.1487511659\dots$ (Boltzmann counting) and $2.3072314551\dots$ (Bose
counting).

\item \textbf{Fisher zeros} (\S\ref{sec:analytic}). $Z_\cT$ is algebraic in $P$, so
its complex-$\beta$ singularities are inherited from $P$ --- which carries a branch
point at $\beta = \rho/k$ for every nontrivial Riemann zero $\rho$, with the imaginary
axis a natural boundary --- plus new branch points where $P(\beta) = \tfrac14$. We
compute the five with $0 < \operatorname{Im}\beta < 50$: they recur near integer
multiples of $2\pi/\log 2$ (the lightest primon sets the frequency) and do \emph{not}
lie on a vertical line. The naive tree analogue of the Riemann Hypothesis is false;
the right question is the distribution of these Fisher singularities --- and that
...
Jessen--Tornehave theory of almost periodic functions). Each singularity occurs when
a single primon mode outshines the coherent rest, and the density is the total
...
\item \textbf{The additive bridge} (\S\ref{sec:bridge}). Fireballs act as
...
Catalan tree with a Gaussian, $\log\log x$, number of primons (Erd\H{o}s--Kac lifted
to fireballs). The interface where the theory genuinely meets the complexity of
...
The Fock-level reading is classical: the primon gas of Julia~\cite{julia} and
...
$\{\log p\}$ and locates a Hagedorn catastrophe at $\beta = 1$. Hagedorn's limiting
temperature and the statistical bootstrap are from~\cite{hagedorn,frautschi}, with the
...
the free magma and its Catalan combinatorics are standard~\cite{stanley}; the
...
its Hagedorn point); the exact critical value $Z_\cT(\sigma^{*}) = \tfrac12$ and the
attainability dichotomy; the computation and non-collinearity of the Fisher zeros;
the process/state/expectation stratification of the arithmetic laws; and the internal
...
%======================================================================
\section{The fireball algebra}\label{sec:trees}

\subsection{Primons and fusion histories}
...
\physread $L_p$ is a single primon, of energy $\varepsilon_p = \log p$. A tree is a
fireball that remembers how it was assembled: $T_1 \boxtimes T_2$ is the fusion of the
fireballs $T_1$ and $T_2$, in that order. Examples of \emph{distinct} microstates:
\[
L_2, \qquad
...
\emph{absence} of axioms. Keeping the left--right order is keeping the fusion channels
ordered, as one does for planar diagrams of matrix-valued fields~\cite{thooft};
keeping the parenthesization is keeping the interaction history --- which pair fused
first. A multiplication tree is the parse tree of a multiplication expression before
evaluation; equivalently, a tree-level diagram built from a single cubic vertex whose
external legs are primons.
...
semigroup $(\NN_{\ge2},\cdot)$ sending $L_p$ to $p$.
\end{definition}

\physread Define the energy $E(T) = \log N(T)$. Then $E(L_p) = \varepsilon_p$ and
$E(T_1 \boxtimes T_2) = E(T_1) + E(T_2)$: energies add under fusion \emph{because}
shadows multiply. The shadow is the calorimeter reading --- the single conserved
...
value $6$ occurs only as the common reading of the two fireballs $L_2\boxtimes L_3$
...
free \emph{commutative} magma (trees with unordered branches); $\cM$ is the free
commutative semigroup (nonempty finite \emph{multisets} of primes). The shadow map
...
\emph{Forgetting the history} keeps only the ordered list of constituent primons ---
the asymptotic content of the fireball, an S-matrix state with the internal dynamics
integrated out. \emph{Symmetrizing} declares the primons indistinguishable: Bose
...
\begin{proposition}[Planar fireballs]\label{prop:gT}
\[
...
\begin{proposition}[Non-planar fireballs]\label{prop:gB}
...
$S_\cW(12) = \log 3$ for the beam, $S_\cB(12) = \log 2$ non-planar, and $0$ in Fock
space. And $g_\cT(12) = C_2 \cdot \tfrac{3!}{2!\,1!} = 2\cdot 3 = 6$, as predicted.
\end{example}
...
%======================================================================
\section{Partition functions: bootstrap equations and the Hagedorn ladder}
\label{sec:zeta}
...
physically, the single-particle (single-primon) partition function. For each corner
...
\]
the Euler product \emph{is} the textbook free-boson partition function --- this is the
primon gas of Julia~\cite{julia}.
...
\emph{(\ref{eq:cat}) is a statistical bootstrap equation} in the sense of
Hagedorn--Frautschi~\cite{hagedorn,frautschi}: \emph{a fireball is either an input
particle or a fusion of two fireballs}, with energy exactly conserved; here the input
spectrum is the primons and the fusion is ordered and binary.
\emph{(\ref{eq:otter}) is the non-planar bootstrap}: unordered fusion, with the
...
\subsection{The Hagedorn ladder}

\begin{theorem}[Inverse Hagedorn temperatures]\label{thm:ladder}
...
$T_H = 1/\sigma_{\mathcal X}$. Remembering order and history multiplies the microstate
count at each energy, raises the entropy per unit energy, and lowers the Hagedorn
temperature:
...
non-planar fireball & unordered histories & $1.988477\dots$ & $0.502897\dots$ & cusp, $Z_\cB \to 1$ \\
planar fireball & ordered histories & $2.597385\dots$ & $0.385003\dots$ & cusp, $Z_\cT \to \tfrac12$ \\
\bottomrule
\end{tabular}
\end{center}

\begin{remark}[Pole versus cusp: is the Hagedorn temperature attainable?]
\label{rem:cusp}
...
susceptibility, rather than a divergent partition function. Whether the Hagedorn
temperature is attainable --- the classic question of the bootstrap
literature~\cite{nahm} --- is here decided by a single algebraic choice:
...
\begin{remark}[The classical bootstrap on the primon spectrum]\label{rem:bootstrap}
The historical statistical bootstrap model fuses in one shot: a fireball is an input
particle \emph{or an unordered cluster of $k \ge 2$ fireballs}, Boltzmann-weighted by
$1/k!$. Its generating equation is $G = \varphi + (e^{G} - 1 - G)$, whose square-root
...
\qquad G_c = \log 2 .
\]
Transplanted onto the primon spectrum ($\varphi = P$), this gives the inverse Hagedorn
temperatures
\[
...
language, the fireball world $\cT$ is a bootstrap model whose decay chains are fully
...
%======================================================================
\section{Fisher zeros of the arithmetic gas}\label{sec:analytic}

Lee and Yang taught that the thermodynamics of a system is encoded in the complex
singularities of its partition function --- in complex fugacity for them, in complex
temperature for Fisher~\cite{leeyang,fisher}. We now locate the complex-$\beta$
singularities of the fireball gas. Since $Z_\cT = \tfrac12(1 - \sqrt{1-4P})$ is an
...
\begin{enumerate}
\item Lifting arithmetic to fireballs does not dissolve the Riemann Hypothesis. The
...
contributes its own --- computable --- critical singularities on top.
...
P(s) = \tfrac14,
\end{equation}
square-root branch points of $Z_\cT$ --- the Fisher singularities of the fireball gas.
...
primon}, $\varepsilon_2 = \log 2$, whose Boltzmann factor $2^{-\beta}$ dominates $P$;
...
naive tree analogue of the Riemann Hypothesis --- Fisher singularities on a vertical
...
\begin{theorem}[Density of the Fisher singularities]\label{thm:density}
...
which is once more unique factorization. By Kronecker--Weyl the flow
...
\]
by residues, and the boundary event has probability zero since the law of $X_\sigma$
is absolutely continuous (already the sum of two circle modes is).
\end{proof}

\physread Each Fisher singularity occurs at a moment when a \emph{single primon mode
outshines the coherent sum of all the others} (together with the constant
$-\tfrac14$); the density of singularities is the total rate of mode dominance. At
$\sigma_1 = 1.30$ the Monte Carlo values are $q_2 = 0.624$, $q_3 = 0.148$,
$q_5 = 0.042$, $q_7 = 0.017$ (Appendix~\ref{app:verification}): the lightest primon
accounts for only about $59\%$ of the singularities, which is exactly why the
observed density \emph{exceeds} the naive one-per-period guess $\log 2/2\pi$.

The numbers confirm the theorem with pleasing force. A census to height $500$
...
between two entirely independent computations (Newton root-finding on
$\texttt{primezeta}$ against a random-phase Monte Carlo) --- while one-per-period
...
map $\cT \to \cA$. \emph{Multiplication acts on quantities} --- fireballs are
...
associative by construction --- sequences --- and one may further quotient $\cG$ by
...
primon is recovered as the observable $\alpha$ of its heap. The foundational question
...
describes exactly what a random integer looks like (\S\ref{sec:randomfireball}). We
...
freeness means the fireball layer is polynomial-time syntax over its input spectrum.
...
\subsection{The random fireball}\label{sec:randomfireball}

What the microstate view \emph{does} compute is the statistics of a typical integer
seen at full resolution.

\begin{theorem}[Random fireball]\label{thm:randomfireball}
...
standard Gaussian.
\item[(iii)] Consequently, conditionally on $\Omega(n) = k$ the fireball is exactly a
...
primons), a shape drawn from the \emph{universal} Catalan ensemble carrying no
arithmetic information at all --- that is Proposition~\ref{prop:nofreelunch}(ii)
...
even though, as we now discuss, deterministically \emph{aiming} at a prime is not
known to be.
...
it measures how expensively a fireball can be synthesized once additions are allowed
...
a polynomial bound on the integer roots of a polynomial in terms of its program size
--- implies $\mathrm{P}_{\mathbb C} \neq \mathrm{NP}_{\mathbb C}$~\cite{shubsmale}.
In the vocabulary of this paper: \emph{the computational hardness of arithmetic is a
statement about the additive--multiplicative interface of \S\ref{sec:bridge} ---
about the cost of synthesizing specific fireballs with additions --- and Wilson's
...
(synthesize each primon additively, then fuse), and $\delta(n)$ is the saving that
...
\begin{question}[Fisher-zero distribution: beyond the almost periodic region]
\label{prob:zeros}
...
directory of this repository (\texttt{verify\_combinatorics.py}, standard library
only; \texttt{analytic.py}, requiring \texttt{mpmath} and \texttt{numpy}).
...
\end{aligned}
\]
For the classical bootstrap of Remark~\ref{rem:bootstrap} on the primon spectrum:
...
The census of \S\ref{sec:density} (script \texttt{zero\_census.py}) extends the scan
to $[1.30, 2.72] \times [0.3, 500]$ with primes up to $10^5$ (grid
...
in \texttt{verification/zeros\_t500.txt}. The running counts are
