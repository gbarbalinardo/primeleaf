...
shadow of any tree, and the empty state exists only in the Fock description.
...
tree gas obeying the bootstrap equation $Z = P + Z^2$. The inverse Hagedorn
temperatures form a strict ladder
\[
1 \;<\; 1.3994333287263303 \;<\; 1.9884768518009081 \;<\; \sigma^{*} = 2.5973851271346716 ,
\]
one rung per layer of remembered structure. At its Hagedorn point the tree gas
...
multiples of $2\pi/\log 2$ (the lightest mode, of energy $\log 2$, sets the
frequency) and do \emph{not}
...
\subsection{Modes and fusion histories}
...
fusion of $T_1$ and $T_2$, in that order. Examples of \emph{distinct} microstates:
\[
L_2, \qquad
...
external legs are labeled by primes.
...
Hagedorn--Frautschi~\cite{hagedorn,frautschi}, whose composite particles were called
\emph{fireballs}: a fireball is either an input particle or a fusion of two
fireballs, with energy exactly conserved. Here the input spectrum is the primes and
the fusion is ordered and binary.
...
P(s) = \tfrac14,
\end{equation}
square-root branch points of $Z_\cT$: the Fisher singularities of the tree gas.
...
(synthesize each prime additively, then fuse), and $\delta(n)$ is the saving that
