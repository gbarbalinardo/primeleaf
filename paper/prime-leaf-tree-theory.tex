...
$1 < 1.3994\ldots < 1.9885\ldots < \sigma^{*} = 2.5974\ldots$, and whether the
limiting temperature is attained is decided by associativity: the symmetrized gases
have poles, the tree gases square-root branch points, with
$Z_\cT(\sigma^{*}) = \tfrac12$ exactly. The complex-temperature zeros of the tree gas
...
$\operatorname{Re}\beta > 1$), and a census of $595$ zeros to height $5000$ reveals
a spectrum far more rigid than Poisson or random-matrix statistics. The Fundamental
...
confirms the density and reveals a spectrum markedly more rigid than either Poisson
or random-matrix statistics
...
literature~\cite{nahm}, is decided here by associativity alone. The square-root
...
lighter-mode resonance displaces a zero. It is not the GUE statistics conjectured
for the Riemann zeros themselves~\cite{montgomery}: although the tree gas inherits
the Riemann zeros through $P$, its spectrum has entirely different rigidity. Making
the empirical rigidity
...
signature of the zeros of an almost periodic function rather than of a random-matrix
ensemble.}
\label{fig:spacings}
\end{figure}
