...
\bottomrule
\end{tabular}
\caption{The dictionary. ``Calorimeter'' and ``cluster'' are imagery. ``Primon'',
``fireball'', ``Hagedorn temperature'' and ``Fisher zeros'' are the historical names
(Julia's primon gas, Hagedorn's statistical bootstrap, Fisher's complex-temperature
zeros); we use plainer terms as primary and give these once, where the corresponding
work is cited. Every other row is an exact identity established in the text.}
...
particle or a fusion of two composites, with energy exactly conserved, so
$Z = P + Z^2$. This is the statistical bootstrap of
Hagedorn--Frautschi~\cite{hagedorn,frautschi} (whose composites were the
\emph{fireballs}), here with input spectrum the primes and fusion ordered and binary.
\emph{(\ref{eq:otter}) is the unordered version}: unordered fusion, with the
