...
setting of Julia's primon gas and of Bost--Connes. In the usual reading, an integer
...
parenthesized record of pairwise fusions, whose only conserved quantity is the
...
axiom. The complex-$\beta$ singularities of the tree gas (its Fisher zeros) are
...
recorded fusion history of the primes involved, ordered and parenthesized,
...
tree theory & physics \\
\midrule
prime $p$ & a mode of energy $\varepsilon_p = \log p$ (Julia's ``primon'') \\
multiplication tree $T \in \cT$ & a recorded history of pairwise fusions (Hagedorn's ``fireball'') \\
...
\bottomrule
\end{tabular}
\caption{The dictionary. ``Calorimeter'' and ``cluster'' are imagery; ``primon'' and
``fireball'' are the historical names from Julia's primon gas and Hagedorn's
statistical bootstrap, used below only where those models are cited; every other row
is an exact identity established in the text.}
...
point at $\beta = \rho/k$ for every nontrivial Riemann zero $\rho$, and has the
imaginary axis as a natural boundary), plus new branch points where $P(\beta) = \tfrac14$. We
...
the original $T_H \approx 160$\,MeV. Transplanted onto the primon spectrum
...
Weighted by speed, the lightest primon accounts for only $59\%$ of the
...
the cost of synthesizing specific fireballs with additions, and Wilson's
